...
def single_image_amount(bb_real_8, bb_pred_8, class_idx):
    bb_real_class = [bb for bb in bb_real_8 if bb[0] == class_idx]
    bb_pred_class = [bb for bb in bb_pred_8 if bb[0] == class_idx]

    return len(bb_real_class), len(bb_pred_class)

def single_image_tag(bb_real_8, bb_pred_8, class_idx):
    real_tag = 0
    pred_tag = 0
    ...
    if len(bb_real_class) > 0:
        real_tag = 1
    
    if len(bb_pred_class) > 0:
        pred_tag = 1
...
def metric(image_directory, working_folder, interval, fps, threshold=0.5, class_indexes=[2]):
    ...
    for class_idx in class_indexes:
        ...
        for image_filename in image_filenames:
            ...
            real_amount, pred_amount = single_image_amount(bb_real_8, bb_pred_8, class_idx)
            real_amounts.append(real_amount)
            pred_amounts.append(pred_amount)

            real_tag, pred_tag = single_image_tag(bb_real_8, bb_pred_8, class_idx)
        ...
        class_result["RAM"] = real_amounts
        class_result["PAM"] = pred_amounts

        class_result["RTA"] = real_tags
        class_result["PTA"] = pred_tags
