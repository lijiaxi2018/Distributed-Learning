...
class KVClusterV3:
	...
	def tell_and_distance(self, key):
		"""
		Return the cv whose corresponding cluster center c is closest to the given key.
		"""
		key = np.array(key)
	...
	def convert_to_normal_list(self, array_list):
		return [(a.tolist(), b.tolist()) for a, b in array_list]
	
	def convert_to_numpy_array(self, list_of_tuples):
		return [(np.array(a), np.array(b)) for a, b in list_of_tuples]
	
	def save_model(self, path):
		para = {}
		para['n'] = self.n
		para['kv_pairs'] = self.convert_to_normal_list(self.kv_pairs)
		para['clusters'] = self.convert_to_normal_list(self.clusters)
		para['labels'] = self.labels.tolist()

		with open(path, 'w') as file:
			json.dump(para, file, indent=4)
	...
	def load_model(self, path):
		...
		self.n = para['n']
		self.kv_pairs = self.convert_to_numpy_array(para['kv_pairs'])
		self.clusters = self.convert_to_numpy_array(para['clusters'])
		self.labels = np.array(para['labels'])