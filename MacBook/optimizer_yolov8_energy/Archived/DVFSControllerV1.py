...
from ThroughputPQ import ThroughputPQ

class DVFSControllerV1:
    def __init__(self, efficiency_desc_rate, exploration_desc_rate, GPU_FREQ, CPU_FREQ, efficiency_array):
    ...
    def update_model_helper(self, cpu_freq, gpu_freq, real_throughput, real_power):
        ...
        if self.visited[self.GPU_FREQ.index(gpu_freq)][self.CPU_FREQ.index(cpu_freq)] == False:
            ...
            if rounded_throughput not in self.throughput_dict:
                self.throughput_dict[rounded_throughput] = ThroughputPQ()
            self.throughput_dict[rounded_throughput].add(((cpu_freq, gpu_freq), real_power))
    ...
    def suggest_combination_helper(self, required_throughput):
        ...
        if required_throughput not in self.throughput_dict:
            ...
            if random.random() > exploration_boundary:
                ((cpu_freq_pq, gpu_freq_pq), _) = self.throughput_dict[required_throughput].top()
    ...
    def print_throughput_dict(self):
        # Print the elements in throughput_dict with ascending throughput values
        for throughput in sorted(self.throughput_dict):
            print(f"Throughput: {throughput}")
            self.throughput_dict[throughput].print_elements()