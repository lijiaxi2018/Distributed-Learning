...
def calculate_movement_result_v1(one_clip_result, fps):
	...
	for pair_results in one_clip_result:
		...
		size_sum = sum(pair_results[4][i] for i in range(pair_amount))
		...
		avg_lists['size'].append(size_sum / pair_amount)
	...
	return [np.mean(avg_lists[key]) for key in ['left_top', 'right_top', 'left_bottom', 'right_bottom', 'amount', 'confidence', 'iou', 'size']]
