    ...
    for class_idx in class_indexes:
        ...
        for i in range(clip_number):
            ...
            for fps in fpss:
                ...
                # Handle Object Amount
                ram_clip = single_metric_result[str(class_idx)]["RAM"][idx:idx+frames_per_clip]
                pam_clip = single_metric_result[str(class_idx)]["PAM"][idx:idx+frames_per_clip]

                am_diff_list = []
                for ram_clip_idx in range(len(ram_clip)):
                    if (ram_clip[ram_clip_idx] == 0) and (pam_clip[ram_clip_idx] == 0):
                        am_diff_list.append(1.)
                    else:
                        am_diff_list.append(abs(ram_clip[ram_clip_idx] - pam_clip[ram_clip_idx]) / (0.5 * (ram_clip[ram_clip_idx] + pam_clip[ram_clip_idx])))
                
                am_acc_avg = 1. - np.average(np.array(am_diff_list))
                fps_result["AM_ACC"] = am_acc_avg

                # Handle Object Tagging
                rta_clip = np.array(single_metric_result[str(class_idx)]["RTA"][idx:idx+frames_per_clip])
                pta_clip = np.array(single_metric_result[str(class_idx)]["PTA"][idx:idx+frames_per_clip])
                ta_diff_list = abs(rta_clip - pta_clip)
                ta_acc_avg = 1. - np.average(ta_diff_list)
                fps_result["TA_ACC"] = ta_acc_avg
