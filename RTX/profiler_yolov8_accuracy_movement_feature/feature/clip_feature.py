...
def combine_frame_result_low_level_feature(one_clip_result, fps):
	np_result = np.array(one_clip_result)
	one_clip_average_result = [list(np_result[:, i]) for i in range(len(np_result[0]))]
		
	return one_clip_average_result

def clip_feature(working_folder, results_filename, interval, fpss, class_indexes=[2]):
	print(f"Processing: {results_filename}")
	feature_frame_results = {}
	for fps in fpss:
		feature_frame_results[fps] = load_json_file(os.path.join(working_folder, f"Feature_Dup_I{interval}_F{fps}.json"))
